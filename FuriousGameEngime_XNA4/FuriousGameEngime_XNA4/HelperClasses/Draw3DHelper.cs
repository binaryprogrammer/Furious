...
namespace FuriousGameEngime_XNA4.HelperClasses
{
    static public class Draw3DHelper
    {
        ...
        public static void ChangeEffectBackToBasicEffect(Model model, BasicEffect basicEffect)
        {
            // Table mapping the original effects to our replacement versions.
            Dictionary<Effect, BasicEffect> effectMapping = new Dictionary<Effect, BasicEffect>();
            ...
            foreach (ModelMesh mesh in model.Meshes)
            {
                ...
                foreach (Effect oldEffect in mesh.Effects)
                {
                    ...
                    if (!effectMapping.ContainsKey(oldEffect))
                    {
                        ...
                        // a different texture each time, so we need a fresh copy each
                        // time we want to set a different texture into it.
                        BasicEffect newEffect = (BasicEffect)basicEffect.Clone();
