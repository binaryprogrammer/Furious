...
namespace FuriousGameEngime_XNA4.Cameras
{
    public class QuaternionCamera : Camera
    {
        Quaternion _quaternion = Quaternion.Identity;

        public QuaternionCamera(GraphicsDevice graphicsDevice, float fieldOfView, float minimumViewDistance, float maximumViewDistance)
            : base(graphicsDevice, (float)graphicsDevice.Viewport.Width / (float)graphicsDevice.Viewport.Height, fieldOfView, minimumViewDistance, maximumViewDistance)
        {
            _projection = Matrix.CreatePerspectiveFieldOfView(fieldOfView, _aspectRatio, minimumViewDistance, maximumViewDistance);
        }
